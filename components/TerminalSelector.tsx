  ...
  return (
    <Card className="shadow-lg transition-all duration-300 hover:shadow-xl bg-white/90 border border-slate-100">
      <CardHeader className="pb-3">
        ...
        <div className={`mt-3 text-sm rounded-xl px-4 py-3 border ${serviceType === "drop_off"
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-blue-800">
                      {(() => {
                        ...
                        return terminal
                          ? `We'll drop you off at Terminal ${terminal.terminalCode}. ${terminal.terminalCode === '3'
                ...
                {terminalMeetingPoints.map((terminal) => {
                  ...
                  return (
                    <button
                      key={terminal.id}
                      type="button"
                      className={`w-full text-left rounded-2xl border transition-all duration-200 p-4 bg-white/70 hover:shadow-md hover:-translate-y-[1px] ${isSelected
                    ...
                    >
                      <div className="flex items-start gap-3 sm:gap-4">
                        <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-emerald-100 rounded-xl flex items-center justify-center">
                          <Plane className="h-5 w-5 sm:h-6 text-emerald-600" />
                        </div>
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
                            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                              ...
                              <Badge variant="outline" className="bg-white text-[10px] sm:text-xs text-slate-600 border-slate-200 py-0 sm:py-0.5">
                                {terminal.terminalCode.includes('3') ? 'Domestic' : 'International'}
                              </Badge>
                            </div>
                            {isSelected && (
                              <div className="hidden sm:flex w-6 h-6 rounded-full bg-primary/10 border border-primary/30 items-center justify-center shrink-0">
                          ...
                          <div className="flex items-start gap-2 text-xs sm:text-sm text-slate-700">
                            ...
                            <span className="font-medium line-clamp-2">{terminal.locationDescription}</span>
                          </div>

                          <div className="flex items-center gap-2 text-[10px] sm:text-xs text-slate-600">
                            <Clock className="h-3 w-3 text-slate-500 shrink-0" />
                            <span className="truncate">
                              Pick-up: {terminal.arrivalTimeOffsetMin}-{terminal.arrivalTimeOffsetMax} min after departure
                            </span>
                          </div>

                          <div className="text-[10px] sm:text-xs text-slate-500 bg-slate-50 rounded-lg p-2 mt-2 border border-slate-100">
                            <p className="font-semibold text-slate-700 mb-0.5">Meeting Instructions:</p>
                            <p className="leading-relaxed">
                              {terminal.terminalCode === '3'
                                ? "Domestic parking building (East Lobby)."
                                : "2nd floor arrival pick-up point (look for sign)."
                              }
                            </p>
                          </div>
                        </div>
