...
export function BookingStatusCard({ bookingCode, initialStatus }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchStatus = async (nextCheck: boolean) => {
      if (cancelled) return
      if (status?.whatsapp_sent) return
      if (nextCheck && checks >= MAX_CHECKS) return
  ...
  const showResend = !status?.whatsapp_sent && checks >= MAX_CHECKS
  ...
  const waStatusLabel = status?.whatsapp_sent ? "Sent to WhatsApp" : "Sending to WhatsApp"
  const waStatusColor = status?.whatsapp_sent
    ...
    <Card className="border border-slate-100 shadow-md">
      ...
      <CardContent className="space-y-3">
        <div className={`w-full rounded-xl border px-4 py-3 text-sm font-semibold flex items-center gap-2 ${waStatusColor}`}>
          ...
          <span>{waStatusLabel}{loading && !status?.whatsapp_sent ? " (checking...)" : ""}</span>
        </div>

        {status?.whatsapp_last_error && !status.whatsapp_sent && (
          <div className="flex items-start gap-2 text-xs text-red-700 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
            <AlertTriangle className="h-4 w-4 mt-[2px]" />
            <p>Auto-send failed: {status.whatsapp_last_error}. Ticket is still recorded.</p>
          </div>
        )}
